...
const FuncInstr::ISAEntry FuncInstr::isa_table[] =
{
    ...
    { nullptr   , 0x00,     0x00, FuncInstr::NO_FORMAT, FuncInstr::NO_TYPE }
};
...
FuncInstr::FuncInstr( uint32 bytes)
{
    ...
    int i = 0;
    for ( i = 0; isa_table[i].name != nullptr; i++)
    {
    ...
    assert( isa_table[i].name == nullptr);
}
...
string FuncInstr::Dump( string indent) const
{
    ...
    if ( isa_table[isa_table_num].format == FORMAT_R)
    {
        oss << bytes.asR.s_reg << ' ' << bytes.asR.t_reg << ' ' << bytes.asR.d_reg;
    } else if ( isa_table[isa_table_num].format == FORMAT_I)
    {
        oss << bytes.asI.s_reg << ' ' << bytes.asI.t_reg << ' ' << bytes.asI.imm;
    } else if ( isa_table[isa_table_num].format == FORMAT_J)
    {
        oss << bytes.asJ.offset;
    } else
    {
        assert( 0);
    }
